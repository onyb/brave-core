...
IOSBraveComponentInstallerPolicy::IOSBraveComponentInstallerPolicy(
...
bool IOSBraveComponentInstallerPolicy::VerifyInstallation(
    const base::DictionaryValue& manifest,
...
bool IOSBraveComponentInstallerPolicy::
    SupportsGroupPolicyEnabledComponentUpdates() const {
  return false;
}

bool IOSBraveComponentInstallerPolicy::RequiresNetworkEncryption() const {
  return false;
}

update_client::CrxInstaller::Result
IOSBraveComponentInstallerPolicy::OnCustomInstall(
    const base::DictionaryValue& manifest,
    const base::FilePath& install_dir) {
  return Result(InstallError::NONE);
}

void IOSBraveComponentInstallerPolicy::OnCustomUninstall() {}

void IOSBraveComponentInstallerPolicy::ComponentReady(
    const base::Version& version,
    const base::FilePath& install_dir,
    std::unique_ptr<base::DictionaryValue> manifest) {
  ready_callback_.Run(
      install_dir, GetManifestString(std::move(manifest), base64_public_key_));
}

base::FilePath IOSBraveComponentInstallerPolicy::GetRelativeInstallDir() const {
...
void IOSBraveComponentInstallerPolicy::GetHash(
...
std::string IOSBraveComponentInstallerPolicy::GetName() const {
  return name_;
}

update_client::InstallerAttributes
IOSBraveComponentInstallerPolicy::GetInstallerAttributes() const {
...
void RegisterComponent(component_updater::ComponentUpdateService* cus,
  ...
  auto installer = base::MakeRefCounted<component_updater::ComponentInstaller>(
      std::make_unique<IOSBraveComponentInstallerPolicy>(
